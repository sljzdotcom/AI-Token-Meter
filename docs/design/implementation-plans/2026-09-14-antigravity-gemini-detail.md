...
- [x] **步骤 1：写解析红灯。** 四行与两行输出都断言只得到 `Gemini · Five hour`、`Gemini · Weekly`；四行 fixture 中 Claude/GPT 80% 已用不能成为 primary/usedRatio。增加半组额外行、缺 Gemini 和重复行拒绝测试。
- [x] **步骤 2：运行 Swift/Rust Gemini 解析测试并确认因旧四项输出而失败。**
- [x] **步骤 3：实现最小解析变更。** 严格验证两条必需 Gemini 行及零或完整两条额外行，随后只保存、排序 Gemini 指标。
- [x] **步骤 4：写旧缓存红灯。** 四项缓存归一化并重算；不完整 Gemini 缓存清空可见额度。
- [x] **步骤 5：运行缓存测试确认旧实现保留 Claude/GPT 或旧主指标而失败。**
- [x] **步骤 6：实现 Swift/Rust 缓存归一化并重跑定向测试至通过。**
- [x] **步骤 7：提交额度口径检查点。**
...
- [x] **步骤 1：写补充解析红灯。** `/model` 的 Gemini TSV 返回显示名，第三方或多行省略；`models` 忽略固定 banner，过滤第三方，计算数量并把 High/Medium/Low 变体折叠为系列。
- [x] **步骤 2：运行 Swift/Rust 定向测试并确认缺少解析器或字段而失败。**
- [x] **步骤 3：实现跨平台 `AntigravityCLIInfo` 与纯解析函数。** 限制字符串、条目和系列数量，保持顺序。
- [x] **步骤 4：写采集生命周期红灯。** 断言固定 `/usage`、`/model`、`models` 命令；补充命令各自失败仍返回 fresh 额度；取消继续传播。
- [x] **步骤 5：运行采集测试确认旧实现没有附加命令而失败。**
- [x] **步骤 6：实现两个有界可选命令，保留私有目录、受控环境、输出上限和取消语义。**
- [x] **步骤 7：更新快照复制、隐私清洗及 Rust 所有结构字面量，重跑定向测试至通过。**
- [x] **步骤 8：提交 CLI 信息检查点。**
...
- [x] **步骤 1：写合同和界面红灯。** Fixture 只含两条 Gemini 指标及 CLI 信息；详情断言两额度、模型摘要、版本与更新时间可见，Claude/GPT 与 Credits 不可见；无补充字段仍可渲染。
- [x] **步骤 2：运行合同、SwiftUI 与 Vitest 定向测试，确认旧四卡界面和缺字段失败。**
- [x] **步骤 3：更新 JSON Schema、Swift/Rust/TypeScript 合同验证。**
- [x] **步骤 4：实现 macOS 紧凑 CLI 信息卡和收紧高度，补齐三语言。**
- [x] **步骤 5：实现 Windows 紧凑 CLI 信息卡和样式，补齐双语。**
- [x] **步骤 6：重跑定向测试和真实浏览器详情场景至通过，保持青绿强调色与深色底板。**
- [x] **步骤 7：提交详情与合同检查点。**
...
- [x] **步骤 1：记录红绿证据、实现范围、隐私边界、平台限制和提交索引。**
- [x] **步骤 2：运行 Swift 完整测试和 Release App 构建。**
- [x] **步骤 3：运行 Windows 前端完整测试/生产构建、Rust完整测试/严格 Clippy。**
- [x] **步骤 4：运行共享合同、真实浏览器、文档、公开安全和差异检查。**
- [x] **步骤 5：请求独立代码审查，修复全部 Critical/Important，记录 Minor 结论并重跑受影响验证。**
- [x] **步骤 6：更新台账为已完成并提交最终证据。**
- [x] **步骤 7：按已有本地整合授权合入最新 main，并在合并结果上重新运行必要门禁；不推送、不建 Tag、不发布。**
- [x] **步骤 8：向协调任务回传实现、测试/限制、审查、提交/合并 SHA 和发布状态。**
