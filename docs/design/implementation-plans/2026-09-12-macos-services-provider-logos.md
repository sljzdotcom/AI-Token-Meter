...
- [ ] 先写失败测试，证明现有 ProviderLogo 只能输出白色样式，无法在浅色 Settings 中使用系统前景色。
- [ ] 为 ProviderLogo 增加保持默认白色的可选 tint/style，并证明四个 Provider 的资源、18pt框和现有光学校正不变。
- [ ] 运行 ProviderLogo 与现有浮动条渲染专项，提交实现。
...
- [ ] 先写失败的真实宿主测试，断言四个分组目前缺少 Logo。
- [ ] 用统一组件渲染18pt Logo、6pt间距与产品名称，Logo使用`.primary`并隐藏于辅助功能树。
- [ ] 在浅色/深色真实窗口中核对可见、未裁切，辅助功能只朗读一次名称；运行 Settings 与本地化回归并提交。
...
- [ ] 运行 Services/Logo/真实窗口专项、`scripts/test.sh`、`scripts/build-app.sh`、`scripts/check-docs.sh`和`git diff --check`。
