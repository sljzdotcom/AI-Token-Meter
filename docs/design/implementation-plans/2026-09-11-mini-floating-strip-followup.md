...
- [ ] 写失败测试，锁定 `mini=65/48/10`、`compact=78/48/10`、`comfortable=108/60/12`；默认与旧 JSON 的 `compact` 仍解码为 Compact。
- [ ] 运行 Swift、Vitest 与 Rust 偏好测试，确认旧实现因缺少 Mini、Compact 仍为 65 以及 schema 字段缺失而失败。
- [ ] 将三端密度扩为三档；`automaticallyCollapses` 默认 `true`，旧 schema 补 `true` 并保留延迟值。
- [ ] 重跑定向测试，覆盖 Mini 往返、旧 Compact 不迁移、窗口逻辑宽度和 1×/1.25×/1.5×/2×换算。
- [ ] 提交偏好和尺寸检查点。
...
- [ ] 写失败测试：关闭自动收起会取消待执行计时并展开；关闭期间离开不折叠；重新开启使用保留的 150/800ms；桌面、全屏、Settings、详情和拖动门禁优先级不变。
- [ ] 运行 Swift/Rust 状态机测试并记录旧逻辑仍排程收起的失败。
- [ ] 两端控制器在偏好变更时取消计时并立即展开；仅在开关为真时接受折叠排程。
- [ ] 重跑状态机、窗口位置、多显示器与桌面可见性专项。
- [ ] 提交状态机检查点。
...
- [ ] 写失败测试：Settings 顺序 Mini/Compact/Comfortable；开关关闭时 Stepper 禁用且值未改；Mini 四服务 65×386 且四环周界在路径内；Compact 宽 78；主体悬停不显示齿轮，底弧悬停或键盘焦点显示。
- [ ] 运行 macOS 视图、渲染和 Settings 专项，确认准确失败。
- [ ] 以密度属性决定路径和内容尺寸；将齿轮状态拆为 Settings 区 hover 与焦点，弧线改为深色，按钮与弧区共享连续命中容器。
- [ ] Settings 增加 Mini 和自动收起 Toggle，并以 `.disabled(!automaticallyCollapses)` 控制两个 Stepper。
...
- [ ] 写失败 Vitest：三档 option；开关禁用延迟输入但保留值；Mini/Compact 宽 65/78；主体 pointer enter 不显示齿轮，Settings zone 才显示；focus-visible 可达。
- [ ] 运行 Vitest，确认现有两档和整条 hover 选择器不满足合同。
- [ ] 实现三档映射、独立 Settings zone、深色弧线，并删除 `.floating-strip:hover` 对齿轮可见性的控制。
- [ ] 更新 Settings 三档、自动收起复选框、禁用延迟输入和中英文文本。
...
- [ ] 更新共享 fixture 为 Mini 65、Compact 78、Comfortable 108，并确认旧生产代码准确失败。
- [ ] Chromium 覆盖三档、左右边、1–4 Provider；逐环采样周界；验证过渡帧、主体/底弧 hover、键盘焦点和关闭自动收起后的稳定展开。
- [ ] 运行完整 Swift、Vitest/build、Rust test/fmt/严格 Clippy、文档、公开安全与无 Widget Release App 签名门禁。
- [ ] 独立差异审查并修复 Critical/Important；记录 Minor、开发日志、台账和证据。
- [ ] 提交最终候选并在既有合并授权内完成 PR/main 双平台 CI 与合并；不得创建 tag、Release 或更新源。
