...
- [ ] 修改macOS偏好、设置结构与共享合同测试，要求只存在两档且旧`mini`解码为Compact。
- [ ] 修改Windows前端、浏览器与Rust偏好测试，要求设置只显示两档且旧`mini`归一为`compact`。
- [ ] 运行专项测试并确认旧实现因仍暴露Mini而失败。
...
- [ ] 等待精确main双平台CI通过，从干净且与远端一致的main创建`v0.9.1`。
- [ ] 等待正式发布workflow生成并公开双平台签名资产，推进三个稳定更新入口。
