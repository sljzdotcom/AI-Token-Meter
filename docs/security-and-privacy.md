...
应用通过已安装并登录的官方 `agy` CLI 执行固定的 `-p /usage`、`-p /model` 与 `models` 命令。这些入口只读取额度、当前模型和模型目录，不发送自然语言提示，也不触发模型生成。版本检查和三项读取使用同一受控环境与权限为 `0700` 的私有空目录；应用保留真实 HOME 让官方 CLI 恢复既有 Google 登录，但不读取、复制或修改账户配置、OAuth 凭据和旧 Gemini CLI 设置。

采集只接受已验证的 1.x 版本（最低 1.1.28），限制每条命令的运行时间与输出大小，拒绝 API Key、代理、动态库和启动脚本等环境覆盖，并把日志定向到随采集销毁的临时目录。正常 CLI 仍会连接官方认证、额度和模型目录服务；业务缓存只保存两个 Gemini 额度窗口的已用百分比、重置时间、状态，以及有限长度的当前 Gemini 模型、可用数量、去重系列与 CLI 版本，不保存原始 CLI 输出、第三方模型信息或账户身份。附加信息查询独立失败，不能使已成功的额度查询降级。旧 Gemini CLI 0.58.0 转录继续作为历史发布证据，不再参与生产采集。[Gemini优先规格](design/specifications/2026-09-14-antigravity-gemini-detail-design.md)。
...
| OpenAI Codex 本机活动聚合 | 随统一快照写入 `Application Support/AI Meter` | 新快照覆盖或用户删除缓存 |
| Claude Code 本机活动聚合 | 随统一快照写入 `Application Support/AI Meter` | 新快照覆盖或用户删除缓存；只含日期、计数和模型 ID |
...
- Widget 进程不发网络请求、不运行 Claude Code/OpenAI Codex CLI、不读取 DeepSeek Keychain，也不访问 WebKit 登录会话；
- 主应用在刷新成功或本地基准变化后构建最小展示快照，再请求 WidgetKit 更新；
- 共享 JSON 不包含 API Key、Bearer Token、Cookie、邮箱、手机号、登录表单、CLI 原始输出或 OpenAI Codex 重置券兑换 ID；
...
- Claude Code：由 Claude Code CLI 连接其官方服务；
