...
修正后的测试使用`ContinuousClock`和锁保护的时间点捕获器，继续要求：超时事件不早于50毫秒、调用返回`timedOut`，并且真实超时停止请求到操作系统进程退出少于一秒。夹具由2秒改为30秒，避免延迟触发时终止失效仍靠自然退出通过；独立审查的变异验证在延迟触发且禁用停止操作时证明原2秒夹具会约2.006秒后误通过。公开初始化器仍不带观察者；生产命令、超时、信号、0.5秒强制终止、输出和错误语义均未改变。
