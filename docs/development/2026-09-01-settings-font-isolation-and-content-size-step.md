...
- `d334321 fix: keep symbols at content scope size`：审计并保护继承内容字体作用域的 SF Symbols，避免其被 `+1pt` 放大。
...
2026-09-01 09:50 +0800 结果：185 个测试、38 个套件通过，0 失败。
...
| 项目 | SHA-256 |
| --- | --- |
| 候选 `dist/AI Meter.app/Contents/MacOS/AIMeterApp` | `b6505ae1ab6fd7c5688615af7a81b4b1705ff24d4f20ccd211f95d2aa2efe359` |
...
- DeepSeek 截图显示产品文字使用 Antonio；Logo、圆环/图表几何、刷新 SF Symbol、深海背景和浮动条轮廓未出现随 `+1pt` 改变的可见放大。精确 `+1pt` 与符号零偏移另有 14 个 TypographyTests 的断言证据。
...
- Mission Control、普通 Space、跨 App 层级、真实指针拖动和多显示器均是此前桌面层功能的独立人工门禁，不是本字体任务完成项；本次没有将它们标为通过或回退旧实现。
