...
以上数字不与Windows相加计算通过率。当前前端为133项组件测试；宿主Rust为268项；密度门禁覆盖25项进程生命周期、8个Antigravity详情状态、9种Provider详情表面、16个浮动条布局、2个收起态和608个文字角色，并核对五页签窄窗可达、两档固定顺序、已展开密度即时调整、自动收起关闭后延迟控件禁用且值不变、底部无Settings入口和键盘焦点。旧 Gemini ConPTY、分片和终端输入夹具已随生产交互路径移除；Claude/Codex仍使用的PTY与Windows原生进程门禁保持。
