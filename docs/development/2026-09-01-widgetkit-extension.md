...
| 共享快照 | `5bbb6df` | Builder/Store/隐私回归通过 |
| 主应用发布 | `7e47981` | 发布时机、失败隔离和深链元数据通过 |
| Widget 界面 | `59f21c7` | Small/Medium/Large、时间线和源码安全合同通过，真实 target 编译成功 |
| 条件打包 | `ccd998a` | 无 Widget 构建、签名缺失保护、脚本与 entitlement 合同通过 |
| plist 键路径修复 | `831106f` | 用 PlistBuddy 正确替换/读取带点号 entitlement 键，并锁定回归 |
| 文档与验收说明 | `16f366f` | 补齐安装、故障排查、架构、隐私、测试与发布文档 |
| 降级状态加固 | `df0d22e` | 固定恢复三服务顺序、清除过期重置信息，并为三种尺寸补齐非颜色状态提示与语义色 |
