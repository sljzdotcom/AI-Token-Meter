...
当前稳定通道版本为 `0.5.0`、macOS build `13`，tag 为 `v0.5.0`；macOS 与 Windows 使用同一个 `VERSION`、tag 和 GitHub Release。稳定 appcast、Windows stable latest.json 与旧 Windows Preview feed 同步推进，避免旧机器无法发现更新。公开资产与签名验证见[本版发布记录](2026-09-07-v0.5.0-release.md)。真机验收和代码签名证书状态继续独立记录，不因稳定通道发布而自动变为完成。
...
scripts/package-update-release.sh 0.5.0 13
```
