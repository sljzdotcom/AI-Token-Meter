...
- 当时自动聚合桥接没有识别官网拆分后的返回格式，12 秒后按设计显示降级提示，因此该阶段没有生成本地 30 天历史缓存；此后已由 [Codex 详情与 DeepSeek 同步日志](2026-08-31-codex-deepseek-details.md) 独立修复并完成真实官网验收；
...
- `10a33b1`：实施计划；
- `4dad89b`：提供商详情焦点策略；
- `226d316`：可交互详情面板与控制器接入；
- `493c374`：功能分支以非快进方式合并到 `main`。

## 最终主分支验证

- `main` 合并后重新运行 103 个测试，24 个套件 0 失败；4 个本机环境集成检查保持既有跳过状态；
- 重新完成 release 构建、Info.plist、严格签名、arm64 架构与空白检查；
- `/Applications/AI Meter.app` 与 `main` 构建产物的主可执行文件 SHA-256 一致；
- 重启最终安装版后，DeepSeek 隔离登录会话保留，详情直接进入官方 `/usage` 页面。
