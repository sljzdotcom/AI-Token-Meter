...
AI Token Meter `0.6.3` 同时支持 Apple Silicon Mac（macOS 14 或更新版本）与 Windows 11 x64。从源码构建的共同依赖是 Git，平台依赖分别为：
...
0.6.3发布完成后，从 [v0.6.3](https://github.com/sljzdotcom/AI-Token-Meter/releases/tag/v0.6.3) 下载 `AI-Token-Meter-0.6.3-macOS-arm64.zip` 和同名 `.sha256`，并在下载目录验证：

```bash
shasum -a 256 -c AI-Token-Meter-0.6.3-macOS-arm64.zip.sha256
```
...
Windows 从同一 [v0.6.3 Release](https://github.com/sljzdotcom/AI-Token-Meter/releases/tag/v0.6.3) 下载 `AI-Token-Meter-0.6.3-windows-x64-setup.exe` 与同名 `.sha256`；不要从第三方网盘取得 Windows 版本。安装器是 current-user NSIS，不要求管理员权限；取得 Authenticode 证书前 Windows 可能显示 SmartScreen，请先确认发布页域名和 SHA-256。应用内更新 archive 另有 Tauri minisign 验证。

发布完成后，已有 macOS `0.2.x` 或更高版本、Windows 稳定版及旧 Preview 无需重新下载：Settings → About → **Check for Updates** → **Update Now**。三条更新入口将共同指向 `0.6.3`；设置与凭据的存储位置保持不变。
