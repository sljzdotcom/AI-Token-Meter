...
AI Token Meter 稳定版支持 Apple Silicon Mac 与 macOS 14 或更新版本；当前已公开的 `0.3.0-preview.1` 同时提供 macOS Apple Silicon 与 Windows 11 x64 预览资产，`0.3.0-preview.2` 是下一发布候选。从源码构建的共同依赖是 Git，平台依赖分别为：
...
稳定使用可从 [v0.2.2](https://github.com/sljzdotcom/AI-Token-Meter/releases/tag/v0.2.2) 下载 `AI-Token-Meter-0.2.2-macOS-arm64.zip`；测试当前已公开的双平台 Preview 可从 [v0.3.0-preview.1](https://github.com/sljzdotcom/AI-Token-Meter/releases/tag/v0.3.0-preview.1) 下载对应 macOS ZIP 和同名 `.sha256`。在下载目录验证：
