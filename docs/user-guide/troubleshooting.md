...
## Google Antigravity 状态

- CLI 未安装：核对是否安装了官方 `agy`；旧 `gemini` 可执行文件不会被采用。
- 需要登录：按 Settings 官方指南运行 `agy` 完成 Google 登录后重新检查；应用不会自动打开登录流程。
- 版本不支持：需要 1.1.28 或更高的 1.x 版本，不自动升级或降级已有 CLI。
- 配置不支持：Windows WSL、API Key、代理、动态库或启动脚本等环境覆盖会被拒绝；保留原配置，不为显示额度绕过组织限制。
- 无额度或输出无法识别：不是 0% 或无限额，不应据此重复安装或登录。缓存仍显示上次采集时间，本次错误单独保留。

[数据来源和完整限制](providers.md#google-antigravity)。
...
已知 `0.5.0` 边界：npm 的 Codex 包装器与 Node 安装在不同目录时，应用受限环境可能无法找到 Node。若终端 `codex --version` 正常，不要因此重复安装或重新登录。0.5.1 已发布显式 Node + 官方 npm JS 入口与一致错误分类修复；请先更新到 0.5.1 或更高版本，再在服务卡点击检查状态。证据见 [CHANGELOG](../../CHANGELOG.md) 和[开发验证记录](../development/2026-09-08-windows-cli-post-login.md)。
...
1. Windows 先在 Settings → About 核对已安装版本；下述显式同步、关闭恢复与复用聚焦从明确列出 `REQ-20260904-006` 的 `0.3.0-preview.3` 起可用。
...
- Windows 先安装 `0.3.0-preview.3` 或更新版本，再完全退出并重新打开 AI Token Meter；旧进程不会自动获得新的窗口焦点能力；
...
- 完全退出并安装 `0.3.0-preview.3` 或更新版本；旧进程不会载入新的紧凑密度与原生控件配色；
