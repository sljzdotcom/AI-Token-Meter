...
AI Token Meter 不提供“始终置顶”开关。浮岛和详情按设计不会覆盖普通应用或全屏应用；返回桌面后仍会使用此前保存的侧边与垂直位置。Mission Control 中不悬在 Space 缩略图上方也是设计目标，但当前候选尚未完成该场景的直接实机验收；如果发现异常悬浮，请记录 macOS 版本和复现步骤。
