...
- CLI 未安装：核对是否安装了官方 Gemini CLI；网页或桌面应用不代表 CLI 可用。
- 需要登录：按 Settings 官方指南在 CLI 完成普通 Google OAuth 登录后重新检查；应用不会自动打开登录流程。
- 版本不支持：本轮只验证 0.58.0，不自动升级或降级已有 CLI。
- 配置不支持：Windows WSL、API/Vertex/企业或加密认证、已有系统配置、自定义启动工具等均可能受限；保留原配置，不为显示额度绕过组织限制。
- 无额度或输出无法识别：不是 0% 或无限额，不应据此重复安装或登录。缓存仍显示上次采集时间，本次错误单独保留。

[数据来源和完整限制](providers.md#gemini未发布)。
