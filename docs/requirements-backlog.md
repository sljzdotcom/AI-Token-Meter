...
| REQ-20260908-012 | 下一阶段 / Gemini 与产品管理 | 当前任务全部完成后考虑增加 Google Gemini 用量与第四个 Logo 按钮；颜色与现有三个产品区分，Settings 可选择显示哪些产品并调整按钮位置/顺序，双平台同步 | 中 | 待用户确认 | 2026-09-08 | 公开资料与本地架构调研已完成，尚未实施。等待用户确认：CLI优先范围（先验证无模型调用的额度采集）、青绿或玫红配色、旧用户升级后Gemini默认隐藏。官方额度自动读取仍未实证，不能以本机tokens替代；确认后开发入口制定计划并实现，功能完成再接续015，不发布 | [调研与待确认设计](design/specifications/2026-09-08-gemini-provider-design.md) · 基线6501068 · 文档自检；未进行真实账号操作或产品测试 |
