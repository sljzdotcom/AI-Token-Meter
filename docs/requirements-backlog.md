...
| REQ-20260909-007 | Google CLI接入迁移 | 用户反馈原Gemini CLI已过时、目前改用Google Antigravity CLI，要求在应用中修改对应接入 | 高 | 进行中 | 2026-09-09 | REQ-008发布收尾后自动接续。用户现明确授权开发在本机验证已安装并已登录的Antigravity CLI，候选命令为`agy`；先核对实际路径、来源、版本、帮助和只读登录/额度能力，确认CLI身份及与Gemini的关系。可使用现有登录进行官方支持的只读状态/用量查询，不主动发送模型请求、不重新登录/退出或改动账号配置；秘密和个人账户/用量不进入公开日志、fixture或截图。不只改名或混用额度；若并存产品涉及第四产品身份/额度范围重大改变，回传推荐方案供决定。本项不自动发布 | 2026-09-09用户补充“这台机器已经顺利安装好了Antigravity CLI也login成功了，你可以直接测试一下，启动命令应该是agy”；安装/登录为用户报告，尚待开发实测；官方替代关系及额度读取仍不能宣称已验证 |
