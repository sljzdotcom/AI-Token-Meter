...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260909-004 | 双平台Logo版本发布 | 用户在收到顶部应用Logo本地实现完成但未发布的回传后，明确要求“让开发直接发布新版本” | 高 | 进行中 | 2026-09-09 | 开发入口接续发布包含002的下一稳定版本；目标为本项目公开仓库sljzdotcom/AI-Token-Meter，按既有流程完成必要代码推送、PR/原生CI、审查合并、版本标签、双平台签名安装包、公开Release及三个更新源。门禁失败先修复，不重复发布0.6.1，不上传秘密、不覆盖用户安装；发布后匿名下载、哈希/签名和更新发现核验，回传版本与链接 | 用户本次直接发布指令；本地交付169f4a7，公开前版0.6.1；0.6.2/build17远端标签与Release未占用；协调只登记派工，验证与发布全部由开发入口执行 |
...
| REQ-20260909-002 | 菜单面板品牌展示 | 用户要求截图中菜单弹出面板顶部AI Token Meter软件名旁增加应用Logo，融入现有背景，不突兀 | 中 | 已完成 | 2026-09-09 | 2026-09-09完成：macOS使用32pt现有应用图标和10pt间距；Windows原生托盘菜单使用同一默认图标的禁用品牌首行。背景、刷新、用量摘要和事件保持；实现、完整本机门禁、逐项审查和本地main整合完成。该变化仍在Unreleased，暂不自动发布新版本 | 规格`2f52437` · 实现`da50eb3` · 本地main整合`d4de77b` · 491项Swift/124项前端/261项Rust、真实浏览器、Release App、合同/文档/公开安全全绿 · [开发记录](development/2026-09-09-menu-panel-app-logo.md) |
