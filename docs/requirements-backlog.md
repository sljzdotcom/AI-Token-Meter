...
| REQ-20260908-015 | 新用户接入 / 四产品一致性 | Gemini 功能完成后全面检查四个按钮及 Settings 接入流程，面向任意新用户而非仅作者三台设备；已安装客户端自动识别，未安装或不可用时清晰提示并给出适当操作，四产品体验一致 | 高 | 进行中 | 2026-09-08 | 2026-09-09完成双平台依赖、状态与恢复动作审计，按推荐方案进入失败先行实现：消除重复检查动作，修正DeepSeek首次保存/历史登录混淆，并让Claude/Codex/DeepSeek详情可达Services；保持Gemini重试/文档和所有安全边界。完成独立审查、双平台门禁及整合后接续已授权发布。 | 发布收尾基线`e3c4071` · 接单`57a8293` · [审计与设计](design/specifications/2026-09-09-four-provider-onboarding-consistency-design.md) · [实施计划](design/implementation-plans/2026-09-09-four-provider-onboarding-consistency.md) |
