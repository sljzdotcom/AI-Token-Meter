...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-010 | macOS 设置缺陷 | 用户反馈设置中的“更新时间”为 5 分钟且无法修改；先核对实际控件含义，排查刷新间隔编辑/保存/生效链路并修复 | 高 | 待处理 | 2026-09-08 | REQ-009 完成后执行，不打断当前开发；验收可修改、立即按既有规则生效且重启后保留，保留合理范围和限流保护；不改软件版本更新策略，暂不发布 | 用户反馈；待复现确认原因，不预判是固定值或绑定故障 |
