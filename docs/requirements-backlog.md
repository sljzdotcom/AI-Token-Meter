...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260909-009 | macOS发布测试可靠性 | v0.6.3正式打包的完整Swift门禁中，Codex超时回收用例在预期超时后因夹具PID文件尚未出现而抛出函数级`transportFailure`，阻断发布 | 高 | 进行中 | 2026-09-09 | 父进程启动点PID捕获已按失败先行实现，本地完整491项Swift、Release App、合同、文档、公开安全及0/0/0复核通过；下一步PR/main双平台CI，全部通过后重启同一0.6.3发布事务 | 修复`60fe1eb` · `dc91bac`首次正式打包在创建tag/Release前停止 · RED为缺少`processDidLaunch`参数 · GREEN单项约1.058秒、组内16/16约17.726秒、完整491项 · 0.6.3/build18 Release App验证通过 · [规格](design/specifications/2026-09-09-codex-timeout-pid-capture-design.md) · [计划](design/implementation-plans/2026-09-09-codex-timeout-pid-capture.md) · [开发记录](development/2026-09-09-codex-timeout-pid-capture.md) |
| REQ-20260909-008 | 双平台新版本发布 | 用户在确认macOS About作者行删除后明确要求发布新版本，供现有设备更新 | 高 | 进行中 | 2026-09-09 | PR #20最终候选和合并提交已通过双平台CI；首次正式打包在创建tag/Release前被REQ-009测试取证竞态拦截。先完成修复PR/main精确CI，再执行签名Release、三个更新源和匿名公网验收。REQ-007不混入本补丁版 | 用户原话“确认发布” · [PR #20](https://github.com/sljzdotcom/AI-Token-Meter/pull/20) · 最终候选`81a8338`：macOS 34353680373、Windows 34353680376 · 合并`dc91bac`：macOS 34354932107、Windows 34354932151 · 首次打包未创建`v0.6.3`tag/Release · [发布记录](development/2026-09-09-v0.6.3-release.md) |
