...
| REQ-20260909-003 | Gemini 安装引导反馈 | 用户无法从Gemini CLI当前按钮获得清晰安装方法，怀疑跳转并非安装教程；需核对链接及按钮语义并修复新用户安装路径 | 高 | 进行中 | 2026-09-09 | 纳入进行中的REQ-20260908-015同类接入审计，不重复开独立实现。两端已改为直达官方安装指南，并明确Node.js 20+、固定支持0.58.0、安装命令、普通Google OAuth登录及安装后重新检查步骤；扩展候选完整本机门禁已通过，等待独立复审与双平台原生CI。真实用户安装/登录仍由用户触发 | 用户反馈“那个按钮点出来好像不是安装方法” · 扩展候选489项Swift、123项前端、259项Rust及Release构建通过；验收新用户可以从按钮直接找到适用安装步骤并完成检测→登录链路，不停在泛首页 |
...
| REQ-20260908-015 | 新用户接入 / 四产品一致性 | Gemini 功能完成后全面检查四个按钮及 Settings 接入流程，面向任意新用户而非仅作者三台设备；已安装客户端自动识别，未安装或不可用时清晰提示并给出适当操作，四产品体验一致 | 高 | 进行中 | 2026-09-08 | 原候选独立复核0/0/0且双平台原生CI已绿；用户随即指出Gemini按钮没有提供安装方法，核对确认两端误开额度说明页。REQ-20260909-003扩展现已完成本机全门禁，等待扩展候选独立复审和双平台原生CI，再接续已授权发布。 | 发布收尾基线`e3c4071` · 接单`57a8293` · 设计`7886a63` · 实现`80da33f` · 审查修复`4149086`/`978a170` · 扩展候选macOS 489项/Release · Windows前端123项/Rust259项/浏览器608角色 · [PR #17](https://github.com/sljzdotcom/AI-Meter/pull/17) · [审计与设计](design/specifications/2026-09-09-four-provider-onboarding-consistency-design.md) · [实施计划](design/implementation-plans/2026-09-09-four-provider-onboarding-consistency.md) · [开发记录](development/2026-09-09-four-provider-onboarding-consistency.md) |
