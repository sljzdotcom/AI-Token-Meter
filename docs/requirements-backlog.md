...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260909-009 | macOS发布测试可靠性 | v0.6.3正式打包的完整Swift门禁中，Codex超时回收用例在预期超时后因夹具PID文件尚未出现而抛出函数级`transportFailure`，阻断发布 | 高 | 进行中 | 2026-09-09 | 父进程启动点PID捕获已按失败先行实现并经PR/main双平台CI和正式发布完整门禁复验；v0.6.3已经成功公开，等待发布后证据提交的精确main双平台CI后结项 | 修复`60fe1eb` · 证据`7e44440` · [PR #21](https://github.com/sljzdotcom/AI-Token-Meter/pull/21) · 合并`dff54d8` · PR CI 34358581690/34358581569 · main CI 34359878983/34359878996 · [workflow 34361767956](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34361767956) · [开发记录](development/2026-09-09-codex-timeout-pid-capture.md) |
| REQ-20260909-008 | 双平台新版本发布 | 用户在确认macOS About作者行删除后明确要求发布新版本，供现有设备更新 | 高 | 进行中 | 2026-09-09 | 0.6.3/build18、七项公开资产及三个更新源已发布并通过匿名下载、哈希、签名和篡改拒绝验证；等待发布后证据提交的精确main双平台CI，再结项并按用户顺序继续REQ-007 | 用户两处明确确认“确认发布”“允许” · [Release v0.6.3](https://github.com/sljzdotcom/AI-Token-Meter/releases/tag/v0.6.3) · [PR #20](https://github.com/sljzdotcom/AI-Token-Meter/pull/20) · [PR #21](https://github.com/sljzdotcom/AI-Token-Meter/pull/21) · 合并/tag`dff54d8` · appcast`fd737f0` · [workflow 34361767956](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34361767956) · [发布记录](development/2026-09-09-v0.6.3-release.md) |
...
| REQ-20260909-007 | Google CLI接入迁移 | 用户反馈原Gemini CLI已过时、目前改用Google Antigravity CLI，要求在应用中修改对应接入 | 高 | 进行中 | 2026-09-09 | 按用户明确顺序，在REQ-008发布结项后继续。先核实官方产品关系、实际CLI包名/安装入口、版本、认证和可合法读取的用量接口；用户关于替代关系的陈述待官方证据确认，不能只改名称或把不同额度混用。按核实结果调整双平台发现、安装/登录引导、采集、展示与旧配置迁移，保留仍有效旧接入并防止数据丢失；如实际为并存产品或需变更第四产品身份/额度范围，回传推荐方案请用户确认重大分歧。不得借用私有OAuth调用未授权后端、不自动迁移账号/安装CLI；常规技术决定自行推进，不自动发布 | 用户明确要求改用Antigravity CLI，并补充“执行完毕后，再继续进行Antigravity CLI的任务”；官方支持/替代关系与额度读取尚待调研，不能标记已完成 |
