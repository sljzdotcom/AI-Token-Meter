...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | macOS计时/真实PTY Suite已拆成独立测试进程，本机458+3+13+6共480项通过。第四轮Windows Gemini失败已定位为测试夹具把ConPTY启动光标握手回复`ESC[1;1R`与固定`/model`业务输入混合，因精确命令永不相等而超时；这与历史Claude runner证据为同类根因。夹具只剥离该协议回复，同时记录原始/业务输入并断言握手存在、清理结果和完整`/model → Esc → /quit`顺序，四种场景及原期限保留。宿主语法、格式、严格Clippy和24项Gemini定向测试通过；Windows专用综合用例仍以PR原生runner复验，不复跑放行、不延长截止、不跳过 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [macOS失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [Windows ConPTY失败34229619374](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619374) · [历史同类证据33719315118](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/33719315118) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
