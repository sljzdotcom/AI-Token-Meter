...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | macOS计时/真实PTY Suite已拆成独立测试进程，本机458+3+13+6共480项通过。第九轮Windows `579c6ec` 的完整合成帧证明ConPTY在模型页重编码中加入12个标准`CSI n X`字符擦除操作，现有小型终端解析器在首个`ESC[125X`返回UnrecognizedOutput。7778字节真实帧与最小ECH语义回归先红后绿；实现只把当前行从光标起最多n个已存在单元改为空格，不扩容且不移动光标。会话级回归又证明旧实现会把登录提示擦掉后错误进入模型页，修复现让ECH与既有行擦除一样先校验当前可见行；最终独立复审Critical/Important/Minor为0/0/0，本任务重新验证252项宿主Rust、严格Clippy、格式、6份合同、207份文档及公开安全门禁全绿，进入提交和原生Windows CI。完整Windows四场景、固定命令、8/2秒期限、未知序列拒绝及子进程回收均保留 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [macOS失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [第四轮Windows失败34229619374](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619374) · [第五轮Windows反证34232052626](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34232052626) · [第六轮Windows字节证据34237111918](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34237111918) · [第八轮Windows分段证据34242730653](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34242730653) · [第九轮Windows完整帧34244232308](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34244232308) · [Windows官方ECH语义](https://learn.microsoft.com/en-us/windows/console/console-virtual-terminal-sequences#text-modification) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
