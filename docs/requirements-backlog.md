...
| REQ-20260903-011 | 发布交付 | 将已合入 `main` 的稳定显示器位置修复及当前跨平台功能打包为下一版可安装 Preview；macOS 与 Windows 使用同一版本和 GitHub Release，提供校验、更新清单及标准发布文档 | 高 | 进行中 | 2026-09-03 | Windows 签名构建已成功；修复 Tauri v2 实际以 NSIS 安装器本身作为 updater 资产的发布流程后，需重跑并完成公开下载复验 | [发布流程](development/release-process.md)、[Release notes](releases/v0.3.0-preview.0.md)、[发布记录](development/2026-09-04-v0.3.0-preview.0-release.md) |
...
| REQ-20260904-002 | Windows 发布缺陷 | 修复 Tauri 已成功生成签名 NSIS 资产后，Release workflow 在资产标准化与哈希阶段无法识别实际输出而中止的问题 | 高 | 进行中 | 2026-09-04 | Runner 证实真实产物为 `setup.exe`/`.exe.sig`；失败先行回归、最小修复及完整本机门禁通过，等待 Windows Release workflow 复验 | [失败 workflow](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/33814393456) · `scripts/test-normalize-windows-release-assets.sh` |
