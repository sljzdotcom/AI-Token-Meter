...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | macOS计时/真实PTY Suite已拆成独立测试进程，本机458+3+13+6共480项通过。设备属性回复归一化后，第八轮Windows `bc813ed` 的分段诊断显示累计输出在16/1778/3859字节分别为Waiting/Ready/Waiting，7778字节模型页帧才由终端解析器返回UnrecognizedOutput；原始输入准确归一化为`/model\r`，尚未发送Esc/quit，证明输入协议、就绪识别及普通137/4096字节分片不是本轮根因。下一步用失败先行回归定位最后3919字节内首个不支持或被ConPTY转换的控制序列，再按真实语义修复并复跑独立审查及原生Windows完整CI；生产固定命令、四种结果、8/2秒期限和子进程回收均保留 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [macOS失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [第四轮Windows失败34229619374](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619374) · [第五轮Windows反证34232052626](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34232052626) · [第六轮Windows字节证据34237111918](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34237111918) · [第八轮Windows分段证据34242730653](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34242730653) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
...
| 2026-09-08 | REQ-20260906-003 | 进行中 | 新增宿主回归把同一真实40KB untrusted模型页按ConPTY最大4096字节及137字节边界分片，两种均取得60%并完成`/model → Esc → /quit`，排除普通读取分片。Windows专用测试现用包装层记录停止前实际输出、非空读取边界及各边界的terminal/quota解析结果，失败只显示总长、96字节头和256字节尾；全部为固定合成数据且有界。等待原生runner区分ConPTY输出转换与中间帧校验问题。 |
