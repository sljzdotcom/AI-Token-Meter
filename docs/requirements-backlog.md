...
| REQ-20260908-011 | 浮动条外观 | 用户已明确要求 Windows/macOS 两端去掉悬浮条顶部小横线，不再需要该提示 | 中 | 进行中 | 2026-09-08 | 2026-09-08 用户批准：仅移除展开态横线装饰及无用样式，保留背景拖动、贴边、位置保存、Logo 布局和折叠态竖线；开发已接续 REQ-010 交付开始真实渲染及交互回归，暂不发布 | 批准基线 `4cf994a` · 开发回传规格/计划 `393a7eb`，正在验证横线移除、拖动及 Provider 点击隔离；待最终交付整合 |
| REQ-20260908-010 | macOS 设置缺陷 | 用户反馈设置中的“更新时间”为 5 分钟且无法修改；先核对实际控件含义，排查刷新间隔编辑/保存/生效链路并修复 | 高 | 进行中 | 2026-09-08 | 开发对话已在 REQ-009 交付后开始诊断；验收可修改、立即按既有规则生效且重启后保留，保留合理范围和限流保护；不改软件版本更新策略，暂不发布 | 用户反馈；开发对话已接续，待原因与测试证据回传 |
