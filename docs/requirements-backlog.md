...
| REQ-20260909-007 | Google CLI接入迁移 | 用户反馈原Gemini CLI已过时、目前改用Google Antigravity CLI，要求在应用中修改对应接入 | 高 | 待用户确认 | 2026-09-09 | 本机只读验证已确认官方`agy` 1.1.28、既有登录、模型列表及无推理`agy -p /usage`均可用；脱敏结构为Gemini与Claude/GPT两组各有Weekly和Five Hour额度。推荐第四项整体改名Google Antigravity并展示全部四窗，内部保留`gemini`兼容ID以无损迁移；AI Credits首期不接入。等待用户确认这项身份与额度范围后按规格TDD实施；本项不自动发布 | 用户授权本机现有登录只读验证；实际额度、邮箱和套餐未进入日志/fixture/截图；[调研与待确认规格](design/specifications/2026-09-09-antigravity-cli-migration-design.md) |
