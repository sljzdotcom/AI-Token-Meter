...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | macOS计时/真实PTY Suite已拆成独立测试进程，本机458+3+13+6共480项通过。第五轮Windows `08dc153` 已通过前端、真实浏览器、生产构建、格式、严格Clippy和前序Rust测试，但Gemini ConPTY成功场景仍在8.88秒超时，反证“只混入固定`ESC[1;1R`”不足以解释阻塞；NSIS/PE未运行。失败现在会在临时目录清理前输出最多六项去重单调阶段历史、总长度及最多128个合成输入字节，避免阶段回退、泄露或无界日志；Node语法、Rust格式/严格Clippy和207份文档通过。等待推送后由原生runner给出真实证据，再按字节修复；完整固定命令、四种结果、期限和子进程回收全部保留，不复跑放行、不延长截止、不跳过 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [macOS失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [第四轮Windows失败34229619374](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619374) · [第五轮Windows反证34232052626](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34232052626) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
