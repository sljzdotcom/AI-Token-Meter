...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260907-001 | 多显示器与拖动 | M4 Max 接入主副两屏后浮动条启动在副屏且无法拖到主屏；修复跨屏拖动，提供明确显示器选择，并设计仅一屏/所有屏显示选择与断屏回退、位置记忆 | 高 | 待用户确认 | 2026-09-07 | 已定位固定侧边忽略横向位移、将焦点屏视为主屏的代码路径；确认是否本轮加入所有显示器模式（涉及多窗口所有权），而非只修复拖动并增加单屏选择。尚未改程序 | [调查与推荐](development/2026-09-07-multidisplay-and-windows-localization.md)；关联 REQ-20260903-009，不能用此前单屏更新成功替代本项验收 |
| REQ-20260907-002 | Windows 本地化与字体 | Windows 增加简体中文/English语言选择（默认英文）；字体增加微软雅黑、黑体、楷体，默认微软雅黑；macOS 不增加语言选择 | 中 | 待处理 | 2026-09-07 | 完成多屏当前阶段后实施；梳理前端、托盘、原生提示与字体缺失回退，保持原有 Settings 系统字体例外规则 | 用户明确 Windows 已安装成功；安装成功不等于全部 Windows 专项验收通过 |
