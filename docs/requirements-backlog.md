...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-003 | Windows 回归测试 | npm/Node 原生回归已执行入口，但 PATH 断言将 Windows 长路径与同一目录的 8.3 短路径直接按字符串比较而失败 | 高 | 进行中 | 2026-09-08 | `3955bb2` 已修正完整 PATH 的规范化比较，定向复审无阻塞；保留单一 Node 目录及入口/参数/退出码断言，待原生 CI 复跑 | [初轮 Windows CI](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34183253409)；process_runner 10 通过/1 失败的边界不隐藏 |
| REQ-20260908-002 | 发布测试回归 | 完整验证发现 appcast 测试固定要求旧版 0.2.2，更新源滚动移除旧项后失败；改为验证真实发布条目的版本、签名与下载路径契约 | 中 | 进行中 | 2026-09-08 | 正按 Task 4 修正测试契约并增加损坏条目负例；不改更新源、不跳过签名/路径校验 | `SoftwareUpdatePackagingTests.stableAppcastContract`；验收：当前有效更新源通过，损坏版本/签名/下载路径失败 |
