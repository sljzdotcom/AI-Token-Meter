...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | macOS计时/真实PTY Suite已拆成独立测试进程，本机458+3+13+6共480项通过。第六轮Windows `69e842d` 的有界诊断显示夹具到达`started>ready-written>input-received`，原始输入为`ESC[?61;6;7;21;22;23;24;28;32;42c/model\r`；Gemini录制输出开头包含`ESC[c`设备属性查询，ConPTY回送的DA响应被夹具误算进业务命令。夹具现用一次原始字符串扫描仅剥离固定光标位置回复与语法完整的主设备属性回复，避免两阶段删除拼接异常片段；5项失败先行Node回归、109项前端、格式、严格Clippy、24项Gemini宿主测试、合同、文档与公开安全门禁通过。等待修复后独立复审及原生Windows完整CI；生产采集代码、完整命令、四种结果、8/2秒期限和子进程回收未改 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [macOS失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [第四轮Windows失败34229619374](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619374) · [第五轮Windows反证34232052626](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34232052626) · [第六轮Windows字节证据34237111918](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34237111918) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
