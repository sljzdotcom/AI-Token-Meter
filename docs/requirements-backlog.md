...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-016 | 双平台新版本发布 | 用户明确要求“先直接发布新版本”，优先发布 Gemini 及当前累积改动，让现有设备可在应用内更新 | 高 | 进行中 | 2026-09-08 | 最新明确发布授权覆盖本轮原先不推送/不发布约束，允许必要开发分支推送、原生CI、修复审查、主分支整合、双平台签名构建与Release/更新源发布。开发入口全程执行；先补齐原生Windows编译/ConPTY等发布门禁，不绕过失败、不把合成账号冒充真账号验收；若仍有阻断或需缩减本版功能，回传重大分歧，不擅自改发布范围。015顺延至本次发布之后，保留未完成项；发布后验证两端更新发现、下载和签名/哈希，回传版本、链接、提交、测试与已知限制 | 用户2026-09-08明确发布指令；Gemini开发交付be2f350、原生Windows验证待补；协调入口仅登记派工，不自行发布 |
