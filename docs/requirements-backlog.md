...
| REQ-20260909-002 | 菜单面板品牌展示 | 用户要求截图中菜单弹出面板顶部AI Token Meter软件名旁增加应用Logo，融入现有背景，不突兀 | 中 | 进行中 | 2026-09-09 | 双平台实现、完整本机门禁与逐项审查已完成，下一步由开发入口整合本地main并补最终Git证据。macOS使用32pt现有应用图标和10pt间距；Windows原生托盘菜单使用同一默认图标的禁用品牌首行。背景、刷新、用量摘要和事件保持；暂不自动发布新版本 | 规格`2f52437` · 实现`da50eb3` · 491项Swift/124项前端/261项Rust、真实浏览器、Release App、合同/文档/公开安全全绿 · [开发记录](development/2026-09-09-menu-panel-app-logo.md) |
...
| 日期 | ID | 变化 | 说明 |
| --- | --- | --- | --- |
| 2026-09-09 | REQ-20260909-002 | 进行中 | 双平台失败先行实现和本机候选门禁完成：macOS真实菜单面板渲染确认32pt前导图标，Windows真实Tauri IconMenuItem确认禁用品牌首行并实际消费RGBA图像；491项Swift、124项前端、261项Rust、真实浏览器、生产构建、严格Clippy、Release App、6份合同、215份Markdown与公开安全全绿。逐项审查无Critical/Important/Minor发现，等待本地main整合后结项；版本、标签和更新源未改。 |
