...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-016 | 双平台新版本发布 | 用户明确要求“先直接发布新版本”，优先发布 Gemini 及当前累积改动，让现有设备可在应用内更新 | 高 | 进行中 | 2026-09-08 | 采用向后兼容新功能版本0.6.0、macOS build15；版本元数据、CHANGELOG、Release Notes与发布日志候选已同步。授权覆盖原先不推送/不发布约束，允许必要分支推送、原生CI、修复审查、main整合、双平台签名构建与Release/更新源发布。独立发布审查的四项Important与一项Minor已按失败先行回归修复，进入完整候选复验；随后推PR补原生Windows编译/ConPTY门禁，不绕过失败或冒充真实账号验收；015顺延到发布后 | 用户明确发布指令 · Gemini本地交付be2f350 · 需求登记30f5384 · 候选8172a2f/b6dcddf · [发布计划](design/implementation-plans/2026-09-08-v0.6.0-release.md) · [Release Notes](releases/v0.6.0.md) · [发布记录](development/2026-09-08-v0.6.0-release.md) |
