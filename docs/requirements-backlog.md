...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-011 | 浮动条外观核对 | 用户询问 Windows/macOS 浮动条顶部小横线的作用及能否去掉；核对装饰与拖动交互是否耦合，避免移除后无法移动 | 中 | 待用户确认 | 2026-09-08 | 已核对为展开态拖动视觉提示；建议两端仅去除横线装饰，保留非按钮背景拖动、贴边与位置保存，不动折叠态竖线。等待用户确认是否实施该视觉调整；当前未改代码，不打断 REQ-010 | macOS FloatingStripView 顶部 Capsule 明确关闭命中；Windows FloatingStrip 横线为 aria-hidden，拖动监听在父容器，独立于横线 |
