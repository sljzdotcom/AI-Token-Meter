...
| REQ-20260909-007 | Google CLI接入迁移 | 用户反馈原Gemini CLI已过时、目前改用Google Antigravity CLI，要求在应用中修改对应接入 | 高 | 进行中 | 2026-09-09 | 用户已确认推荐方案：第四项整体改名Google Antigravity，展示Gemini与Claude/GPT两组Weekly和Five Hour共四个额度窗口，内部保留`gemini`兼容ID，AI Credits首期不接入。按规格及实施计划TDD完成双平台采集、发现、展示、接入引导、迁移、文档和原生CI；本项不自动发布 | 2026-09-10用户原话“按照你的推荐继续执行” · 本机官方`agy` 1.1.28、既有登录和无推理`agy -p /usage`已脱敏验证 · [规格](design/specifications/2026-09-09-antigravity-cli-migration-design.md) · [实施计划](design/implementation-plans/2026-09-10-antigravity-cli-migration.md) |
