...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-011 | 浮动条外观 | 用户已明确要求 Windows/macOS 两端去掉悬浮条顶部小横线，不再需要该提示 | 中 | 进行中 | 2026-09-08 | 隔离分支 codex/remove-strip-drag-hint 接续010交付；2026-09-08 用户批准：仅移除展开态横线装饰及无用样式，保留背景拖动、贴边、位置保存、Logo 布局和折叠态竖线；接续 REQ-010 开发交付后执行，补回归测试与文档，暂不发布 | 批准 `4cf994a` / 引入 `e843750` · [规格](design/specifications/2026-09-08-remove-strip-drag-hint-design.md) · [计划](design/implementation-plans/2026-09-08-remove-strip-drag-hint.md) |
