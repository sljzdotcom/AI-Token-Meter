...
| REQ-20260908-015 | 新用户接入 / 四产品一致性 | Gemini 功能完成后全面检查四个按钮及 Settings 接入流程，面向任意新用户而非仅作者三台设备；已安装客户端自动识别，未安装或不可用时清晰提示并给出适当操作，四产品体验一致 | 高 | 进行中 | 2026-09-08 | 2026-09-09收到0.6.0发布完成回传后，协调按用户原授权接续派工。建立macOS/Windows × 四产品 × 安装/登录/数据状态验收矩阵；统一浮动条、详情与Settings的状态和下一步，保留各提供方真实能力差异、凭据安全与用户触发安装/登录；开发入口负责审计、修复、验证及整合，现场无法复验的边界继续明确记录，本项不自动触发下一次发布。 | 发布收尾基线e3c4071；用户要求Gemini发布后继续；验收：四产品各状态有一致且正确的文案/按钮，成功接入后状态同步刷新，无客户端和已有客户端的新用户都能完成配置 |
