...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | PR #13 第二轮macOS CI中461项主测试通过，单独PTY进程内两个各自serialized的Suite仍并行运行，出现父进程退出超时、并发输出缺失及Gemini ready/PID时序共4项失败；首轮同候选macOS通过且本轮只改Windows脚本。已把两个真实PTY Suite拆为独立测试进程，保留全部19项、原截止与断言；本机461+13+6共480项及流程合同通过，等待原生CI复验跨Suite资源竞争假设，不能用本机成功冒充根因关闭 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [PR #13本次失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
