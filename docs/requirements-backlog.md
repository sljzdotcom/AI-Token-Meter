...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260907-008 | CLI 安装分发核对 | 确认 CLI/安装器是否内嵌软件包、是否增大安装体积；若内嵌大型文件则改为在线下载 | 中 | 进行中 | 2026-09-07 | 核对两平台安装代码与打包资源，先说明实际边界，不重复实现已有在线下载 | 验收：明确点击前后下载内容，不捏造安装包体积数据 |
| REQ-20260907-009 | 关于与品牌展示 | macOS/Windows 关于页在 Miller 作者后增加 Twitter @MillerPanYue 与项目 GitHub 链接、小图标；Windows Settings 顶部名称旁增加现有软件 Logo，与 macOS 呼应 | 中 | 待处理 | 2026-09-07 | 用户授权当前工作完成后直接设计实施，无需再次确认；保留系统字体与现有品牌，链接点击才打开 | 验收：双平台链接准确且可访问，中英文标签清晰，图标不重复读屏，Windows 软件图标为本地资源且不影响布局 |
