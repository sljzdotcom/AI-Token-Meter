...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260909-005 | Windows CI / Tauri测试入口 | v0.6.2 PR新增托盘品牌回归调用Tauri `mock_app()`，原生Windows测试程序在进入测试前以`STATUS_ENTRYPOINT_NOT_FOUND`退出，阻断NSIS门禁 | 高 | 进行中 | 2026-09-09 | 品牌合同已改为不启动MockRuntime并按失败先行转绿，完整261项Rust、格式、严格Clippy及依赖图通过；下一步推送并由PR原生Windows runtime/NSIS/GUI subsystem复验。不得跳过Windows测试或降低CI | [失败Windows CI 34330043004](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34330043004) · 精确提交`da109ba` · Tauri上游`tauri-apps/tauri#13419` |
| REQ-20260909-004 | 双平台Logo版本发布 | 用户在收到顶部应用Logo本地实现完成但未发布的回传后，明确要求“让开发直接发布新版本” | 高 | 进行中 | 2026-09-09 | PR #19首轮macOS CI全绿；Windows在进入NSIS前发现并定位Tauri MockRuntime上游缺陷，失败先行修正及完整261项Rust/严格Clippy已通过。下一步推送修正并等待双平台精确CI，再完成main整合、签名Release及三个更新源 | 用户本次直接发布指令 · 基线169f4a7 · 候选b76585a · 审查da109ba · [PR #19](https://github.com/sljzdotcom/AI-Token-Meter/pull/19) · [发布记录](development/2026-09-09-v0.6.2-release.md) |
