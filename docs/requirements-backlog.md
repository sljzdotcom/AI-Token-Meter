...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-010 | macOS 设置缺陷 | 用户反馈设置中的“更新时间”为 5 分钟且无法修改；先核对实际控件含义，排查刷新间隔编辑/保存/生效链路并修复 | 高 | 进行中 | 2026-09-08 | REQ-009 已回传；隔离分支 codex/macos-refresh-interval 开始诊断；验收可修改、立即按既有规则生效且重启后保留，保留合理范围和限流保护；不改软件版本更新策略，暂不发布 | 已定位静态5分钟标签与300秒固定睡眠，无编辑/持久化链路 · [规格](design/specifications/2026-09-08-macos-refresh-interval-design.md) · [计划](design/implementation-plans/2026-09-08-macos-refresh-interval.md) |
| REQ-20260908-009 | About 界面与社交链接 | Windows 设置→关于移除“作者 Miller”整行和可见“作者链接”标题，仅保留 Twitter/GitHub 并新增带图标的 Telegram @sljzdotcom；macOS 也增加 Telegram 图标链接 | 中 | 已完成 | 2026-09-08 | 2026-09-08 开发交付：Windows 中英文作者整行/可见标题移除，两端 Telegram 固定图标入口完成；未合入 main，待协调入口核验整合，暂不发布。Windows 原生 WebView2/DPI 仍属现场边界 | `a508658` · `fc2f3f9` · `e288f3f` · `e89fe6a` · `23c7824` · 92 前端/221 宿主 Rust/434 Swift、构建及独立审查通过 · [开发日志](development/2026-09-08-about-telegram.md) |
