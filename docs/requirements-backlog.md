...
| REQ-20260908-017 | Windows CI 浏览器稳定性 | 0.6.0 发布 PR 的真实浏览器密度门禁中，GitHub Windows runner 上 Google Chrome 进程在45秒内没有任何stdout/stderr或退出事件，导致后续原生Rust/ConPTY/NSIS门禁未运行 | 高 | 进行中 | 2026-09-08 | 保留失败日志与有界清理，不用成功复跑冒充修复。失败表明外层进程看门狗不能让卡在页面等待的Chromium主动完成dump-dom；按Chrome官方捕获参数增加10秒内部DOM期限，仍保留45秒外层进程期限与失败清理。22项生命周期、实际浏览器8个Gemini详情状态/16组四服务布局/632个文字角色及109项前端、production build已本机通过，等待PR真实Windows完整Rust/ConPTY/NSIS复验，无跳过或无界重试 | [失败 CI 34226445772](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34226445772) · [Chrome Headless捕获期限](https://developer.chrome.com/docs/automation-and-testing/headless-cli#--timeout) · 失败先行回归：内部捕获期限必须早于外层看门狗 |
...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | macOS计时/真实PTY Suite已拆成独立测试进程，本机458+3+13+6共480项通过。精确提交`dbb1fc6`与`935a199`的Windows真实ConPTY四场景连续全绿。第十一轮CI证明单独测试target仍会因293次累计重放超过两秒；会话现先排空非阻塞接口中当前已就绪的所有分片，再对完整累计历史校验一次，并在每次读取前保留取消、deadline和512KiB限制。新安全回归先证明旧实现会在已排队登录提示前错误发送`/model`，修复后无输入即拒绝；137/4096边界、两秒期限、完整命令和60%断言不变，本机分片回归由约0.83秒降至0.04秒。初审及分片覆盖增量复审均0/0/0，本任务完整253项Rust与458+3+13+6共480项Swift、严格Clippy、合同/发布脚本、207份文档和公开安全门禁全绿；准备提交原生CI，NSIS/PE尚未运行 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [macOS失败34227393553](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393553) · [第四轮Windows失败34229619374](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619374) · [第五轮Windows反证34232052626](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34232052626) · [第六轮Windows字节证据34237111918](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34237111918) · [第八轮Windows分段证据34242730653](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34242730653) · [第九轮Windows完整帧34244232308](https://github.com/sljzdotcom/AI-Meter/actions/runs/34244232308) · [第十轮Windows分片超时34247798952](https://github.com/sljzdotcom/AI-Meter/actions/runs/34247798952) · [第十一轮Windows分片反证34249876816](https://github.com/sljzdotcom/AI-Meter/actions/runs/34249876816) · [Windows官方ECH语义](https://learn.microsoft.com/en-us/windows/console/console-virtual-terminal-sequences#text-modification) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
