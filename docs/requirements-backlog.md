...
| REQ-20260909-002 | 菜单面板品牌展示 | 用户要求截图中菜单弹出面板顶部AI Token Meter软件名旁增加应用Logo，融入现有背景，不突兀 | 中 | 进行中 | 2026-09-09 | 收到v0.6.1最终收尾后协调接续派工。沿用既有应用Logo，小尺寸与标题/副标题整体垂直对齐、透明容器、不另加底板/粗框/强阴影，不挤压右侧刷新按钮或改变当前背景；开发先确认截图对应页面，两端等价菜单面板保持品牌一致，重大视觉差异才交用户选择。由开发完成实现/验证/审查/合并，暂不自动发布新版本 | 收尾基线78b1ad3；用户截图菜单面板顶部缺应用Logo；截图路径仅随派工传递，不公开个人用量或截图元数据 |
