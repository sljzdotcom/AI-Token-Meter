...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260908-022 | 发布授权流程 | 用户反馈开发入口对v0.6.0公开推送/合并/Release反复请求授权且自动审批拒绝，要求定位并避免重复阻塞 | 高 | 进行中 | 2026-09-08 | 用户已在开发任务手工要求继续发布；以开发任务收到的具体原话核对本次授权，不重复索取已有确认，也不据此推定长期授权。协调已查到开发任务active/inProgress，正请求简短回传确认运行阶段及通信通道。授权规则须明确公开目标sljzdotcom/AI-Token-Meter及动作，拒绝后不换路径重试；不能承诺审批永不拒绝 | 用户要求核对运行与回传；状态查询显示活动中，但尚不能单凭active宣称发布正常或完成；文档维护/提交由开发入口负责 |
| REQ-20260908-021 | macOS 浮动条渲染测试稳定性 | v0.6.0 精确提交 `08dc153` 的完整本机门禁中，`FloatingStripRenderingTests` 的六个compact/Codex布局检查均得到 `brightLogoPixels == 0`，导致458项普通测试组失败；此前同候选完整门禁通过，本提交未改渲染生产代码 | 高 | 进行中 | 2026-09-08 | 两次聚焦测试稳定重现并输出具体布局；PNG证明Logo存在，但1×位图将compact Codex细线抗锯齿到最高约86%，原92%参考掩码为空。现恢复该测试原有的明确2× Retina采样，并把尺寸、顶部玻璃和Logo坐标全部从点换算成像素；92%/85%阈值、四种provider数量、两种密度、两侧边缘及折叠截图均保留。聚焦矩阵及完整458+3+13+6共480项、合同、发布脚本、207份文档和公开安全检查通过，等待独立复审与PR CI，不用一次本机成功关闭 | 本机 `bash scripts/test.sh` · 两次聚焦红灯及2×绿灯 · `FloatingStripRenderingTests.swift` · 提交 `08dc153` |
| REQ-20260908-020 | macOS 刷新调度 CI 稳定性 | PR #13 第四轮 macOS 主测试中，`RefreshIntervalSchedulingTests.replacesWaitingTimerAndRejectsLateCompletions` 在等待条件处超过测试上限；本轮仅改 Windows 专用测试，上一轮精确候选相同调度测试通过 | 高 | 进行中 | 2026-09-08 | 日志显示大量主线程测试并行启动，许多无关用例同步耗时约8秒；刷新调度用例在获得第一次推进机会前已越过两秒行为窗口。流程合同已先红后绿，把3项主线程调度测试放入独立进程，仍保留替换等待计时器、拒绝旧完成、运行中不重叠的全部断言和原期限；本机458+3+13+6共480项及全部仓库门禁通过，等待独立复审和PR原生复验，不用本机成功关闭 | [失败 CI 34229619225](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34229619225) · `RefreshIntervalSchedulingTests.swift:137` · 失败先行流程合同 |
| REQ-20260908-019 | Windows 条件测试编译 | 0.6.0 发布 PR 的原生 Windows 严格 Clippy 继续编译到 `cli_installation` 测试后，发现安装脚本 URL 断言未处理新增的 Gemini 枚举项；这是 Windows 专用测试代码，macOS 宿主门禁未编译该分支 | 高 | 进行中 | 2026-09-08 | 保留仅 Claude/Codex 可下载执行安装脚本、Gemini 只开放官方指南的产品边界；已把两种可执行脚本与期望 URL 建成显式测试表，避免对完整枚举做不穷尽匹配，没有使用 wildcard、allow、skip 或降级检查。宿主格式、严格 Clippy 与安装定向测试通过；本机虽已安装 Windows Rust target，但缺少 Windows CRT 头文件，交叉编译在第三方 ring 构建阶段停止，仍以独立复审和 PR 原生 Windows 完整门禁放行 | [失败 CI 34228704586](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34228704586) · `windows/src-tauri/tests/cli_installation.rs:123` |
| REQ-20260908-018 | Windows 严格静态检查 | 0.6.0 发布 PR 的原生 Windows 严格 Clippy 在 `service_account_statuses` 的 Windows 条件编译分支发现冗余 `return`，导致后续运行时、ConPTY、Job Object、NSIS 与 PE 门禁未运行 | 高 | 进行中 | 2026-09-08 | 以原生 `-D warnings` 失败作为红灯，已移除冗余返回并保留相同状态收集行为；没有增加 lint allow、跳过或降级严格检查。宿主格式、严格 Clippy、249项测试及文档门禁通过，等待独立复审与 PR 原生 Windows 完整复验所有后续门禁 | [失败 CI 34227393562](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34227393562) · `windows/src-tauri/src/lib.rs` Windows 条件编译分支 |
| REQ-20260908-017 | Windows CI 浏览器稳定性 | 0.6.0 发布 PR 的真实浏览器密度门禁中，GitHub Windows runner 上 Google Chrome 进程两次在45秒内没有任何stdout/stderr或退出事件，导致后续原生Rust/ConPTY/NSIS门禁未运行 | 高 | 进行中 | 2026-09-08 | 保留失败日志与有界清理，不用成功复跑冒充修复。单浏览器增加10秒内部DOM期限仍不足；Windows现优先尝试与WebView2一致的Edge，再在进程级失败后尝试Chrome，且最多两个不同浏览器家族；显式浏览器覆盖也按可执行文件basename/path识别家族并去重。每次仍有45秒外层看门狗、10秒DOM期限、2MiB输出上限、隔离profile和进程树清理；DOM/布局断言只在首个进程成功结果上执行，清理失败会立即阻断而不被回退掩盖。25项生命周期及实际浏览器8个Gemini详情状态、16组四服务布局、632个文字角色已本机通过，等待修复复审与PR原生Windows完整Rust/ConPTY/NSIS复验 | [首次失败 CI 34226445772](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34226445772) · [再次失败 CI 34252424048](https://github.com/sljzdotcom/AI-Token-Meter/actions/runs/34252424048) · [Chrome Headless捕获期限](https://developer.chrome.com/docs/automation-and-testing/headless-cli#--timeout) · 失败先行回归：有界回退、家族去重与清理失败不可掩盖 |
| REQ-20260908-016 | 双平台新版本发布 | 用户明确要求“先直接发布新版本”，优先发布 Gemini 及当前累积改动，让现有设备可在应用内更新 | 高 | 进行中 | 2026-09-08 | 采用向后兼容新功能版本0.6.0、macOS build15；版本元数据、CHANGELOG、Release Notes与发布日志候选已同步。授权覆盖原先不推送/不发布约束，允许必要分支推送、原生CI、修复审查、main整合、双平台签名构建与Release/更新源发布。本地480项Swift、Gemini官方CLI隔离4场景、Windows宿主门禁、macOS Release与公开安全检查通过；独立复核确认四项Important和一项Minor均已关闭，下一步推PR补原生Windows编译/ConPTY门禁，不绕过失败或冒充真实账号验收；015顺延到发布后 | 用户明确发布指令 · Gemini本地交付be2f350 · 需求登记30f5384 · 候选8172a2f/b6dcddf · 审查修复46e6de7 · [发布计划](design/implementation-plans/2026-09-08-v0.6.0-release.md) · [Release Notes](releases/v0.6.0.md) · [发布记录](development/2026-09-08-v0.6.0-release.md) |
| REQ-20260908-015 | 新用户接入 / 四产品一致性 | Gemini 功能完成后全面检查四个按钮及 Settings 接入流程，面向任意新用户而非仅作者三台设备；已安装客户端自动识别，未安装或不可用时清晰提示并给出适当操作，四产品体验一致 | 高 | 待处理 | 2026-09-08 | 排在012功能完成之后，开发 Agent负责审计、修复、验证与文档收尾。建立 macOS/Windows × 四产品 × 安装/登录/数据状态验收矩阵；区分桌面客户端、CLI及API Key依赖，已安装不误报未安装、不要求重复安装/登录；未安装提供明确可选安装引导，未登录/过期提供登录操作，权限/网络/路径异常可诊断而非误归未安装。自动发现常见安装方式和GUI环境路径，不硬编码作者用户名/目录；DeepSeek等无需CLI的模式不提示安装无关客户端。统一浮动条、详情、Settings的状态与下一步，保留各提供方实际能力差异；凭据安全、最小权限、用户触发安装/登录。覆盖干净用户环境与旧配置迁移，记录无法现场验证的边界，不冒充全平台通过，不自动发布 | 用户要求Gemini完成后直接进行；验收：四产品各状态有一致且正确的文案/按钮，成功接入后状态同步刷新，无客户端和已有客户端的新用户都能完成配置 |
...
| REQ-20260908-012 | 下一阶段 / Gemini 与产品管理 | 当前任务全部完成后考虑增加 Google Gemini 用量与第四个 Logo 按钮；颜色与现有三个产品区分，Settings 可选择显示哪些产品并调整按钮位置/顺序，双平台同步 | 中 | 受环境限制 | 2026-09-08 | 2026-09-08已按许可完成本地实现、文档与独立审查：四产品展示/迁移、固定官方Gemini CLI0.58.0普通OAuth额度，Windows首期native。478 Swift、109前端、249宿主Rust、严格Clippy、Release与官方隔离4场景通过；全部审查发现关闭。原生Windows SDK/ConPTY及真实账号未验收，暂不合入main。保留codex/gemini-research-design，由开发入口在环境与授权满足后补验并整合；不推送、不发布。015继续等待012功能完成 | 用户青绿#3ED6B2及本地提交授权 · [规格](design/specifications/2026-09-08-gemini-provider-design.md) · [计划](design/implementation-plans/2026-09-08-gemini-provider.md) · [实现/验证/审查与环境边界](development/2026-09-08-gemini-collector.md) · macOS edd9cdd/f96b08c · Windows 5a0cced/8675152/057e79f；未标记已完成 |
...
| REQ-20260906-003 | CI可靠性 | 追踪偶发的macOS PTY父进程退出超时/并发输出缺失、CLI超时测试PID读取失败及Windows ConPTY采集超时；不删除断言或跳过测试 | 高 | 进行中 | 2026-09-06 | Gemini会话排空修复及分片覆盖增量复审均0/0/0，完整253项Rust与458+3+13+6共480项Swift、严格Clippy、合同/发布脚本、207份文档和公开安全门禁全绿。精确提交`505d6d8`的macOS CI仅父进程退出fixture超时：Python解释器在两秒期限内冷启动约1.59秒。fixture现改用原生shell立即输出并退出，HUP免疫后台sleep继续持有PTY slave；两秒请求期限、三秒总耗时、exit/output/子进程阶段断言及生产PTY代码不变。完整PTY runner 13项与父退出单项连续5次已本机通过，等待独立复审及双平台原生CI；NSIS/PE尚未运行 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 原失败CI `34033000263`、`34033000265`、`34073917970` · [第十轮Windows分片超时34247798952](https://github.com/sljzdotcom/AI-Meter/actions/runs/34247798952) · [第十一轮Windows分片反证34249876816](https://github.com/sljzdotcom/AI-Meter/actions/runs/34249876816) · [本轮macOS失败34252423980](https://github.com/sljzdotcom/AI-Meter/actions/runs/34252423980) · [Windows官方ECH语义](https://learn.microsoft.com/en-us/windows/console/console-virtual-terminal-sequences#text-modification) · [本轮发布记录](development/2026-09-08-v0.6.0-release.md) |
