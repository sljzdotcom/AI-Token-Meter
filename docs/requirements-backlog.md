...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-20260907-006 | CLI 安装与账户引导 | Settings 未安装 CLI 时显示变色安装按钮协助一键安装；已安装自动定位，未登录/过期以变色登录按钮提醒；已登录显示当前账号并提供重新登录或退出登录操作，macOS/Windows 同步 | 高 | 进行中 | 2026-09-07 | 核对官方安装方式和现有账户状态机，采用显式用户点击安装/登录及安装后重新检测；不静默更换现有 CLI 或账户 | 验收：缺失/检测中/未登录/已登录/失败状态明确，按钮可操作且防重复，安装失败可恢复、无凭据泄漏 |
| REQ-20260907-007 | Windows Settings 页签图标 | Windows 设置的外观、监测、服务、关于等页签名称增加图标便于区分，保留中英文文字和系统默认字体 | 中 | 进行中 | 2026-09-07 | CLI 引导任务审查已通过，开始四个 16px 线性图标与中英文交互测试，不改变 macOS 页签 | 验收：四页签均图标+文字，中英文可辨识，键盘焦点与无障碍名称不重复 |
