...
**Google Antigravity 的 Gemini 优先口径已随0.10.1公开：** 第四项保留内部 `gemini` 兼容标识；采集仍由官方 `agy -p /usage` 提供，但快照、圆环、摘要和提醒只采用 Gemini 五小时与每周两个窗口。详情补充当前 Gemini 模型、动态可用数量/系列、CLI 版本与检查时间，附加命令失败不影响额度。[规格](design/specifications/2026-09-14-antigravity-gemini-detail-design.md)。
...
| Google Antigravity 真实账号与实际额度 | 待用户确认 | 用户安装公开版0.10.1后，在已登录 `agy` 1.1.28+ 的设备核对两个 Gemini 窗口、模型摘要、重置时间和刷新；开发阶段已完成脱敏只读 CLI 探测，不展示个人额度 |
