...
vi.mock('@tauri-apps/api/core',()=>({invoke:async(command:string,args?:{providerId?:string,retryUsage?:boolean})=>{
 ...
 if(command==='service_account_status'){
  ...
  return {providerId:args?.providerId,connectionState:backend.fresh&&args?.providerId==='gemini'?'connected':'unavailable',accountDetail:args?.providerId==='gemini'?(backend.fresh?'Official Antigravity quota available':'Antigravity not yet checked'):undefined}
 }
 return []
}}))
import {Shell} from '../Shell'
afterEach(()=>{Reflect.deleteProperty(window,'__TAURI_INTERNALS__')})
it('open Settings follows the same Antigravity collector result without launching another refresh',async()=>{
 Object.defineProperty(window,'__TAURI_INTERNALS__',{value:{},configurable:true})
 render(<Shell/>);fireEvent.click(screen.getByRole('tab',{name:'Services'}))
 expect(await screen.findByText('Antigravity not yet checked')).toBeVisible()
 backend.fresh=true
 await act(async()=>{backend.listeners.get('snapshot-updated')?.({payload:{providerId:'gemini',status:'fresh'}})})
 expect(await screen.findByText('Official Antigravity quota available')).toBeVisible()
